...
/** 默认每页评论数量 */
const defaultPageSize: number = 3
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			if (comments?.success && comments.code === 1) {
				...
				setCommentCount(comments.data.total)
		...
		<div className='md:px-8 p-2 md:py-6'>
			...
			<div className={`flex flex-col`}>
				...
				<div className='p-2 md:p-4 bg-white dark:bg-gray-700 rounded'>
					<div className='flex justify-between items-center dark:bg-gray-700 dark:text-gray-400'>
						<span className='text-xl'>评论</span>
						<span className='text-sm'>-&gt; {commentCount} 条</span>
					</div>
					<div className='py-4'>
						<>
							{/* 评论列表 */}
